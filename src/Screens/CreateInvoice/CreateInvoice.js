...
export default function CreateInvoice() {
  ...
  const user_id = "due4866777878676";
  ...
  const saveInvoice = async () => {
    const data = {
      ...
      invoiceNumber: String(invoiceNumber), // force string
      invoiceTitle: String(invoiceTitle), // force string

      dateCreated: new Date(startDate), // backend expects Date
      amount: Number(parseFloat(finalTotal || 0).toFixed(2)),

      // DueIn: String(endDate), 
      DueIn: new Date(endDate), 
      Status: String(newInvoiveStatus), // force string

      items: selectedItem?.map((item) => item._id) || [],
      taxes: selectedTaxItem?.map((item) => item._id) || [],
    };
    ...
    const fetchInvoice = async () => {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          setStartDate(
            new Date(invoice.dateCreated).toISOString().split("T")[0]
          );
          setEndDate(new Date(invoice.DueIn).toISOString().split("T")[0]);
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView>
        <TouchableOpacity
          onPress={() =>
            navigation.navigate("InvoiceIntro", {
              upInvoiceNumber: invoiceNumber,
              upInvoiceTitle:invoiceTitle,
              upInvoiceStatus: newInvoiveStatus,
              upDueDate: endDate,
        ...
        >
          <View>
            {invoiceTitle ? (
              <Text style={[styles.boxHeading,{fontSize:AppFonts.h4}]}>{invoiceTitle || ""}</Text>
            ):null}

            <Text style={[styles.boxHeading,{fontSize:AppFonts.h4}]}>#{invoiceNumber || "INV001"}</Text>
        ...
        <View style={styles.dataBoxSec}>
          <View style={styles.doubleTextHeading}>
            ...
            <View>
              <Text style={styles.boxHeadingSingle}>
                  ...
                  <View style={styles.singleItemWrap}>
                    ...
                    <View>
                      ...
                      <View>
                        ...
                        <Text style={styles.itemTotalPrice}>
                          Total {selectedCurrency?.currencyCode}{" "}
                          {item.totalAmount}
                        </Text>
                      </View>
                    </View>
              ...
              <FlatList
                ...
                renderItem={({ item }) => {
                  const taxRate = parseFloat(item.taxRate) || 0;
                  const taxAmount =
                    ((itemsTotal - discountAmount) * taxRate) / 100;
        ...
        >
          <View style={styles.doubleTextHeading}>
            ...
            <View>
              ...
              {selectedCurrency ? (
                <Text style={styles.boxDesc}>
                  {selectedCurrency?.country} {selectedCurrency?.currencyCode}
                </Text>
              ) : null}
            </View>
          </View>
        ...
        />
        <Button
          title={invoiceId ? "Update" : "Save"}
          onPress={() => {
            invoiceId ? updateInvoice() : saveInvoice();
          }}
        />
      </ScrollView>
    </View>
